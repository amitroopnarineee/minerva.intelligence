  ...
  return (
    <div className="mn-shell">
      <PageHeader
        ...
        title="Get Started"
        subtitle="Complete these steps to get the most out of Minerva."
      />
      <div className="mn-page flex-1 px-6 pb-6">
        <div className="mn-steps max-w-xl space-y-3">
          {steps.map((step, i) => (
            <div
              key={i}
              className="mn-step flex items-start gap-4 rounded-xl border border-border bg-card p-4"
            >
              {step.done ? (
              ...
              <div className="flex-1">
                <h3 className={`text-sm font-medium ${step.done ? "line-through text-muted-foreground" : ""}`}>
                  {step.title}
                </h3>
                <p className="mt-0.5 text-xs text-muted-foreground">{step.description}</p>
              </div>
              {!step.done && (
                <button className="flex items-center gap-1 rounded-md border px-2.5 py-1 text-xs font-medium text-muted-foreground transition-colors hover:text-foreground">
